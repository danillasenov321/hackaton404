			...
			<main className="container py-8">
				...
				{settingGroups.map((group, groupIndex) => (
					<div key={group.title} className="mb-8">
						<div className="flex items-center gap-4 mb-6">
							<div
								className={`w-12 h-12 rounded-2xl bg-gradient-to-r ${group.color} flex items-center justify-center text-white text-xl shadow-lg`}
							>
								{group.icon}
							</div>
							<div>
								...
								<p className="text-gray-600">
									Настройки {group.title.toLowerCase()}
								</p>
							</div>
						</div>
				...
				<div className="bg-gradient-to-r from-gray-100 to-slate-100 rounded-3xl p-6 text-center">
					<div className="text-gray-600 text-sm">
						<p className="mb-2">
							💡 Для применения некоторых настроек может потребоваться
