            ...
            <div className="light-card rounded-3xl shadow-soft p-6 border border-slate-200/50 glass-effect sticky top-6">
              ...
              <div className="space-y-3 mb-8">
                {categories.map(category => (
                  ...
                  >
                    <div className={`icon-wrapper ${selectedCategory === category.id ? 'bg-white/20 text-white' : 'bg-slate-100 text-slate-600'} text-lg`}>
                      {category.icon}
                    </div>
            ...
            <div className="light-card rounded-3xl shadow-soft p-6 border border-slate-200/50 glass-effect">
              ...
              <div className="grid md:grid-cols-2 gap-4">
                {filteredLocations.map(location => (
                  <div key={location.id} className="bg-white/80 rounded-2xl p-4 hover-lift transition-all group border border-slate-200/50">
                    <div className="flex items-start gap-4">
                      <div className={`icon-wrapper ${
                        location.category === 'park' ? 'gradient-forest' :
                        location.category === 'cafe' ? 'gradient-sunset' :
                        location.category === 'hotel' ? 'gradient-ocean' : 'gradient-lavender'
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-slate-600 text-sm mb-2 truncate">{location.address}</p>
                        
                        <div className="flex items-center gap-4 mb-3">
                          <div className="flex items-center gap-1 bg-amber-50 px-2 py-1 rounded-full">
                            <span className="text-amber-600 text-sm">⭐</span>
                            <span className="font-semibold text-slate-800 text-sm">{location.rating}</span>
                            <span className="text-slate-500 text-xs">({location.reviews})</span>
                          </div>
                        </div>
              ...
              {filteredLocations.length === 0 && (
                <div className="text-center py-12">
                  ...
                  <h4 className="text-xl font-semibold high-contrast-text mb-2">Места не найдены</h4>
                  <p className="text-slate-600 mb-6">Попробуйте выбрать другую категорию</p>
                  <button 
        ...
        <div className="container">
          <div className="flex justify-around py-4">
            {[
              ...
              { id: 'map', icon: '🗺️', label: 'Карта', href: '/map', active: true },
