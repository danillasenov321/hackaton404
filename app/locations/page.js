...
export default function LocationsPage() {
	...
	useEffect(() => {
		async function fetchData() {
			try {
				const [reqObj, reqTypes] = await Promise.all([
					fetch("http://localhost:3000/objects.json"),
					fetch("http://localhost:3000/object_types.json")
				]);

				const objData = await reqObj.json();
					...
					<div className="flex items-center justify-between">
						<div>
							...
							<p className="text-gray-600 text-sm">
								Полный список достопримечательностей ДНР
							</p>
						</div>
								...
								<div className="space-y-2">
									{categories.map(category => (
										...
										>
											...
											>
												{
													locations.filter(l => l.category === category.id)
														.length
												}
											</span>
										</button>
					...
					<div className="lg:col-span-3">
						{dbObj.length > 0 ? (
							<div className="grid gap-4">
								{dbObj.map((item, index) => (
									<div
										key={item.id || index}
										className="bg-white/80 backdrop-blur-sm rounded-2xl shadow-soft p-6 border border-white/30"
									>
										...
										<p className="text-sm text-gray-500">
											Раздел: {dbTypes[item.type] || `Тип ${item.type}`}
										</p>
									</div>
								))}
							</div>
