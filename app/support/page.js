	...
	const faqItems = [
		{
			...
			answer:
				"На странице места нажмите на значок звезды в правом верхнем углу карточки."
		},
		{
			question: "Работает ли приложение оффлайн?",
			answer:
				"Да, вы можете скачать оффлайн карты в настройках для использования без интернета."
		},
		{
			question: "Как оставить отзыв о месте?",
			answer:
				"Перейдите на страницу места и нажмите кнопку 'Оставить отзыв' внизу страницы."
		},
		{
			question: "Можно ли построить маршрут до места?",
			answer:
				"Да, на странице места есть кнопка 'Построить маршрут', которая откроет карту с проложенным путем."
		}
	];
				...
				{activeTab === "faq" && (
					<div className="space-y-4">
						{faqItems.map((faq, index) => (
							<div
								key={index}
								className="bg-white/80 backdrop-blur-lg rounded-3xl p-6 border border-white/50 shadow-soft"
							>
				...
				{activeTab === "methods" && (
					<div className="grid gap-6">
						{contactMethods.map((method, index) => (
							<div
								key={method.title}
								className="bg-white/80 backdrop-blur-lg rounded-3xl p-8 border border-white/50 shadow-soft text-center group hover:scale-105 transition-all duration-300"
							>
								<div
									className={`w-16 h-16 rounded-2xl bg-gradient-to-r ${method.color} flex items-center justify-center text-white text-2xl mx-auto mb-4 shadow-lg group-hover:scale-110 group-hover:rotate-12 transition-all duration-300`}
								>
									{method.icon}
								</div>
